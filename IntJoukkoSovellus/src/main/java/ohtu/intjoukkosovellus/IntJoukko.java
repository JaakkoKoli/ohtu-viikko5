...
public class IntJoukko {
    ...
    public IntJoukko() {
        ...
        for (int i = 0; i < lukujono.length; i++) lukujono[i] = 0;
        alkioidenLkm = 0;
    ...
    public IntJoukko(int kapasiteetti, int kasvatuskoko) {
        if (kapasiteetti < 0) throw new IndexOutOfBoundsException("Kapasitteetti väärin");
        if (kasvatuskoko < 0) throw new IndexOutOfBoundsException("kapasiteetti2");
    ...
    public boolean kuuluu(int luku) {
        for (int i = 0; i < alkioidenLkm; i++)
            if (luku == lukujono[i]) return true;
        return false;
    }
    ...
    public String toString() {
        if (alkioidenLkm == 0) {
            ...
            String tuotos = "{";
            for (int i = 0; i < alkioidenLkm - 1; i++) tuotos += lukujono[i]+", ";
    ...
    public int[] toIntArray() {
        ...
        for (int i = 0; i < taulu.length; i++) taulu[i] = lukujono[i];
        return taulu;
    }
    ...
    public static IntJoukko erotus ( IntJoukko a, IntJoukko b) {
        ...
        for (int i = 0; i < bTaulu.length; i++) z.poista(i);
        return z;
    }
